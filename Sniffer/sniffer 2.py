...
def main(argv):
    ...
    t1=threading.Thread(target=loop,args=(cap,arpres,),name='LoopThread1')
...
#解析数据包
def parse_packet(packet,arpres) :
    ...
    if eth_protocol == 8 :
        ...
        arpres.append(str(eth_addr(e_sender)))
        #print(arpres)
        newsarpres = []             
        #print('去重之后的arp')
        for id in arpres:
            if id not in newsarpres:
                newsarpres.append(id)
        #print (newsarpres)
        print('arp报文总个数')
        print(len(arpres))
        print('去重后个数')
        print(len(newsarpres))

        if (len(newsarpres)/(len(arpres)+len(newsarpres))) <0.15:
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("!!!!!!发现疑似arp攻击!!!!!!!")
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
